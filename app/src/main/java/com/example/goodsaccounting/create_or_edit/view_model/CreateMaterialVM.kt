...
internal class CreateMaterialVM(
        ...
        when (event) {
            ...
            is CreateMaterialEvent.InputStringMinimumQuantity -> blockingIntent {
                if (event.minimumQuantity.startsWith("0") || event.minimumQuantity.isBlank()) {
                    reduce { state.copy(stringMinimumQuantity = "") }
                } else {
    ...
    private fun create() = intent {
        ...
        val isErrorMinimumQuantity =  state.minimumQuantity == 0f || state.minimumQuantity > Float.MAX_VALUE
        if (isErrorName || isErrorMinimumQuantity) {
            ...
            createMaterialUseCase.execute(state, state.imageUrl)
                .onSuccess {
                    ...
                    postSideEffect(CreateMaterialSideEffect.ShowMessage(R.string.network_problems))
                    onError(it)
                }
        }
    }
